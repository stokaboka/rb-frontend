...
function $$$RbTopMenuNavBarDirective( $rootScope, utils )
{
	return {
		restrict: "E",
		template: '<md-toolbar id="RB_TOP_NAVBAR">\
		                    <div class="md-toolbar-tools">\
		                    <md-progress-circular md-mode="indeterminate" class="md-warn" rb-visible="rbCore.loader.busy"  md-diameter="20"></md-progress-circular>\
		                        <md-menu-bar >\
		                            <md-menu ng-repeat="map_section in SITE_MAP.sections | filter: { toolbar: true }">\
			                            <md-button class="md-raised md-primary" ng-click="$mdMenu.open()">\
			                                {{map_section.title}}\
                                            <md-tooltip ng-show="map_section.tooltip">{{map_section.tooltip}}</md-tooltip>\
	                                    </md-button>\
	                                    <md-menu-content>\
...
$$$RbTopMenuNavBarDirective.$inject = [ '$rootScope', 'utils' ];